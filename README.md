...
- 前端工作台做了一轮视觉与布局优化：全屏恢复固定侧栏，窗口模式保留可拖拽工作台，日志默认弱化为折叠入口
- 修复全屏下左侧工作台过窄、按钮裁切、长文件名撑破布局、队列区域被挤压的问题
- 处理区支持多图队列：可拖入多张图片，也可导入文件夹，逐张编辑选区并在队列中切换
- 打码选区交互增强：支持选区缩放、移动、撤销 / 重做，并可把当前选区应用到后续图片
- Pixiv 直传登录态导入增强：浏览器 Cookie 被占用时可走临时登录窗口，并自动补齐 / 检测 `CSRF Token`
- Pixiv 调试快照进一步脱敏，避免把 Cookie、Token、用户标识等敏感内容写入调试报告
- 修复 Pixiv 投稿页标签输入在 token/chip 模式下的确认与误删问题
- 优化 Pixiv 标签确认逻辑与调试日志
- 增加 Pixiv 调试快照导出，方便抓脱敏后的 HTML / JSON / 截图
...
- 这个模式会把 Playwright 使用的浏览器资料目录保存在项目下的 `.pixiv_profile/`
- 这样做是为了让浏览器自动填写模式可以复用本地登录态和草稿页上下文
- `.pixiv_profile/` 已经默认加入 `.gitignore`，但仍然只建议保留在你自己的机器上
...
即便如此，仍然建议在分享前先自己过一眼，不要把本地调试文件长期外传。
...
这些敏感字段不会写进 `webview_config.json`，重启工作台后需要重新填写或重新导入。

另外需要注意：

- 浏览器自动填写模式使用的 `.pixiv_profile/` 会作为本地浏览器资料目录保留
- `tmp_pixiv_diag/` 里的调试文件也会保留在本地，目录默认已加入 `.gitignore`
